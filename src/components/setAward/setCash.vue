<template>
  <div v-loading='loading' class='body'>
    ...
    <el-row class='cont'>
      ...
      <!--Dialog 创建现金-->
      <el-dialog width='25%' title='创建现金' :visible.sync='newMerchantVisible'>
      ...
      <!--修改信息-->
      <el-dialog width='25%' title='修改现金' :visible.sync='resetMerchantVisible'>
  ...
  export default {
    ...
    methods: {
      ...
      find () {
        ...
        // console.log(post);
        this.tableData = [];
        this.util.SendAjax(this, post, 'cashCrmApi/getList').then((res) => {
      ...
      submit () {
        let post = {
          prizeCashName: this.newMerchantForm.name,
          amount: this.newMerchantForm.amount,
          description: this.newMerchantForm.description,
          createrId: 160
        };
        this.util.SendAjax(this, post, 'cashCrmApi/add').then((res) => {
          this.$message({
            message: '创建成功！',
            type: 'success'
          });
      ...
      resetMerchant (id) {
        // console.log(id);
        let post = {
          prizeCashName: this.resetMerchantForm.name,
        ...
        // console.log(post);
        this.util.SendAjax(this, post, 'cashCrmApi/update').then((res) => {
      ...
      changeMessage (msg) {
        let post = {
          cashId: msg.prizeCashId
        };
        this.util.SendAjax(this, post, 'cashCrmApi/getOneCash').then((res) => {
          this.resetMerchantForm.name = res.data.prizeCashName;
          this.resetMerchantForm.description = res.data.description;
          this.resetMerchantForm.ID = res.data.prizeCashId;
      ...
      deleteMessage (msg) {
        ...
        this.$confirm('确认要删除吗?', '提示', {
          ...
          this.util.SendAjax(this, post, 'cashCrmApi/delete').then((res) => {
            this.find();
          });
          this.$message({
            type: 'success',
